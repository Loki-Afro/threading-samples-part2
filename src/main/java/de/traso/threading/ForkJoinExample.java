...
public class ForkJoinExample {
    ...
    static class MaximumFinder extends RecursiveTask<Integer> {
        ...
        protected Integer compute() {
            ...
            final MaximumFinder left = new MaximumFinder(this.data, this.start, this.start + split);
            left.fork();
            final MaximumFinder right = new MaximumFinder(this.data, this.start + split, this.end);
            return Math.max(right.compute(), left.join());
        }

        private Integer computeDirectly() {
            System.out.println(Thread.currentThread() + " computing: " + this.start + " to " + this.end);
